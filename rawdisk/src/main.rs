...
use rawdisk::rawdisk_reader::{RawdiskError, RawdiskReader};

#[derive(Parser)]
struct Args {
    // Paths to raw (dd) disk images
...
fn do_hash<P>(path: P) -> Result<Vec<u8>, RawdiskError>
where
    P: AsRef<str>,
{
    ...
    let image_size_bs_disp = ByteSize::b(rawdisk_reader.image_size).display().iec();

    let mut prev_prog = Instant::now();
    let start = prev_prog;

    while offset < rawdisk_reader.image_size {
        let buf_size = buf.len();
        let read = rawdisk_reader.read_at_offset(offset, &mut buf[..buf_size])?;
...
fn run(args: Args) -> Result<(), RawdiskError> {
