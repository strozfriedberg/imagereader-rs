//! Standalone timing probe for `RawdiskReader::open_with_options` and subsequent reads.
//!
//! Isolates two costs that are otherwise conflated inside a live diskimage-nbd + fls-warm
//! session: (1) the one-time open cost (a single stat/head_object for a raw image), and
//! (2) per-read cold-block fetch cost once the reader is open.
//!
...
use rawdisk::{
    init_tracing,
    rawdisk_reader::{CacheMode, RawdiskReader, RawdiskReaderOptions},
};

#[derive(Parser)]
#[command(about = "Time RawdiskReader::open and sampled reads against a real (e.g. s3://) raw image")]
struct Args {
    /// Path or URL to the raw image (local path or s3:// URL).
    image_path: String,
...
fn main() {
    ...
    let opts = RawdiskReaderOptions {
        s3_concurrency: args.s3_concurrency,
        cache_mem_mib: args.cache_mem_mib,
        cache_mode: make_cache_mode(),
        ..RawdiskReaderOptions::default()
    };

    eprintln!("opening {} ...", args.image_path);
    let open_start = Instant::now();
    let mut reader = match RawdiskReader::open_with_options(&args.image_path, &opts) {
        Ok(r) => r,
        Err(e) => {
...
fn run_concurrent(args: &Args, make_cache_mode: impl Fn() -> CacheMode) {
    ...
    let probe_opts = RawdiskReaderOptions {
        s3_concurrency: args.s3_concurrency,
        cache_mem_mib: args.cache_mem_mib,
        cache_mode: make_cache_mode(),
        ..RawdiskReaderOptions::default()
    };
    let image_size = match RawdiskReader::open_with_options(&args.image_path, &probe_opts) {
        Ok(r) => r.image_size,
        Err(e) => {
        ...
        std::thread::scope(|scope| {
            let handles: Vec<_> = (0..n)
                .map(|i| {
                    ...
                    scope.spawn(move || {
                        let t = Instant::now();
                        let mut reader = match RawdiskReader::open_with_options(&image_path, &opts) {
                            Ok(r) => r,
                            Err(e) => {
