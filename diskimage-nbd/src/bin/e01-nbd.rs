...
use e01::e01_reader::{
    CacheMode, CorruptChunkPolicy, CorruptSectionPolicy, E01Reader, E01ReaderOptions, OpenError,
};
#[cfg(unix)]
...
impl NbdImage for E01Adapter {
    ...
    fn read_at_offset(&mut self, offset: u64, buf: &mut [u8]) -> std::io::Result<usize> {
        self.0
...
fn open_reader(args: &Args, cache_mode: CacheMode) -> Result<E01Adapter, OpenError> {
    E01Reader::open_glob(
        &args.e01_path,
        &E01ReaderOptions {
...
fn run(args: Args) -> Result<(), OpenError> {
    let io_log = match &args.io_log {
        Some(path) => {
            let log = IoLog::open(path)
                .map_err(|e| OpenError::BadPath(format!("{}: {e}", path.display())))?;
