...
const IMAGE: &str = "data/monolithicSparse.vmdk";
...
fn open() -> VmdkReader {
    VmdkReader::open(IMAGE).unwrap()
}

/// Read the whole image front to back through `buf`.
fn read_all(reader: &mut VmdkReader, buf: &mut [u8]) {
    let mut offset = 0u64;
    loop {
...
fn warm_sequential_read(c: &mut Criterion) {
    let mut group = c.benchmark_group("vmdk sequential read (warm cache)");
    configure(&mut group);
...
fn warm_random_read(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("vmdk random read (warm cache)");
    configure(&mut group);
...
fn cold_sequential_read(c: &mut Criterion) {
    let mut group = c.benchmark_group("vmdk sequential read (cold cache)");
    configure(&mut group);
...
fn cold_random_read(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("vmdk random read (cold cache)");
    configure(&mut group);
    group.throughput(Throughput::Bytes((RANDOM_BUF_SIZE * offsets.len()) as u64));
